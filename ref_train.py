...
def parse_args():
    ...
    parser.add_argument('--path', default='./Datasets/R2C7K', type=str, help='path to train dataset')
...
def main(args):
    ...
    ### model ###
    net = refFSPNet_model.Model(args.pretrain, img_size=384)
    ...
    Datasampler, Dataloader = get_dataloader(Dir, 5, batchsize=args.batch_size_per_gpu, mode='train')

    # torch.backends.cudnn.benchmark = True
    
    ### main loop ###
    star_time=time.time()
    for curr_epoch in range(0, 51):
        
        if curr_epoch==25 or curr_epoch==150:
        ...
        for data in Dataloader:
            count += 1
            cuda_device = torch.device('cuda', args.rank)
            img, label, supp_feat, _ = data[0].to(cuda_device), data[1].to(cuda_device), data[2].to(cuda_device), data[3]
            out = net(img, supp_feat)
        ...
        if args.rank == 0 and curr_epoch % 2 == 0:
            ckpt_save_root = "./ref_fspnet/ckpt_save"
