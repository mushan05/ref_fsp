...
import vit
from ref_module.modules import FeatFusion
...
class Converter(nn.Module):
    ...
    def forward(self, token_pair):
        ...
        for index in range(len(token_pair) // 2):
            ...
            f1_ = self.norm_layer_f1(token_pair[index * 2])  # [8,576,768]
            f2_ = self.norm_layer_f2(token_pair[index * 2 + 1])  # [8,576,768]
...
class Model(nn.Module):
    def __init__(self, ckpt, img_size=384):
        ...
        self.featfusion = FeatFusion(channel=self.vit_chs)
    ...
    def forward(self, img, ref_f):
        # B Seq
        B, C, H, W = img.size()
        x = self.encoder(img)  # list 12x[b,576,768] vit有12个block,[b, 576,768], 768=3x16x16,576=(384/16)^2
        x = self.featfusion(ref_f, x)
